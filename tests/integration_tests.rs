...
use cosmwasm_std::{coins, from_binary, Binary, ContractResult, Response};
use cosmwasm_vm::testing::{
    execute, instantiate, mock_instance, mock_instance_with_gas_limit, query,
};
...
fn verify_valid() {
    ...
    let res: Response = execute(&mut deps, mock_env(), mock_info("anyone", &[]), msg).unwrap();
    assert_eq!(
...
fn verify_invalid() {
    ...
    let result: ContractResult<Response> = execute(&mut deps, mock_env(), info, msg);
